...
export async function saveOnboarding(uid:string,data:Pick<UserProfile,"traderStyles"|"markets"|"instruments"|"experience"|"baseCurrency"|"goal"|"emailDigest"|"emailFrequency">&{name?:string;email?:string}){const now=new Date().toISOString();const reference=doc(db,"users",uid);const existing=await getDoc(reference);await setDoc(reference,{...data,onboardingComplete:true,...(!existing.exists()?{createdAt:now}:{}),updatedAt:now},{merge:true})}
