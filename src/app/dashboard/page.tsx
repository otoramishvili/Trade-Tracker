...
function DashboardContent(){
  const {user}=useAuth(); const {activeAccount}=useTradingAccounts();
  ...
  return <main className="app-page dashboard-page">
    <div className="page-heading"><div><p className="eyebrow"><i/>Performance overview</p><h1>Good {new Date().getHours()<12?"morning":new Date().getHours()<18?"afternoon":"evening"}, {user?.displayName?.split(" ")[0]||"trader"}.</h1><p className="page-subtitle">{activeAccount?`Performance for ${activeAccount}.`:"Performance across all accounts."}</p></div><Link className="button" href="/trades/new"><span>＋</span> Add trade</Link></div>
    {!visibleTrades.length?<div className="empty-state"><div className="empty-icon">↗</div><h2>No trades for this account.</h2><p>Add a trade or switch accounts from the sidebar.</p><Link className="button" href="/trades/new">Add your first trade <span>→</span></Link></div>:<>
