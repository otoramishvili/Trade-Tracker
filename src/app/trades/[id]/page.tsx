...
function Detail(){
  ...
  const load=useCallback(async()=>{if(!user)return;try{setTrade(await getTrade(user.uid,id))}catch(err){console.error(err);setError("This trade could not be loaded.")}finally{setLoading(false)}},[user,id]);
  useEffect(()=>{void load()},[load]);
  async function remove(){if(!user)return;setDeleting(true);try{await deleteTrade(user.uid,id);router.push("/trades")}finally{setDeleting(false)}}

  if(loading)return <main className="center-state"><span className="loader"/>Loading trade…</main>;
  if(error)return <main className="app-page"><p className="alert">{error}</p></main>;
  if(!trade)return <main className="empty-state"><div className="empty-icon">?</div><h1>Trade not found</h1><p>It may have been removed or belongs to another account.</p><Link className="button" href="/trades">Back to journal</Link></main>;
  ...
  return <main className="app-page trade-detail-page">
    ...
    <div className="detail-layout">
      ...
      <aside className="detail-side-column"><section className="trade-meta-card"><h3>Entry details</h3><dl><div><dt>Created via</dt><dd><span className="source-dot">{trade.source==="ai_import"?"✦":"✎"}</span>{trade.source==="ai_import"?"AI Import":"Manual Entry"}</dd></div><div><dt>Trade ID</dt><dd className="trade-id">{trade.id}</dd></div>{created&&<div><dt>Created</dt><dd>{created.toLocaleDateString()}</dd></div>}{updated&&<div><dt>Last updated</dt><dd>{updated.toLocaleDateString()}</dd></div>}</dl></section>{trade.originalImportText&&<section className="original-text-card"><span>Original AI import</span><p>{trade.originalImportText}</p></section>}<Link className="side-edit-button" href={`/trades/${id}/edit`}>Edit this trade <span>→</span></Link></aside>
    </div>
...
function EmptySection({text}:{text:string}){return <div className="review-empty"><span>○</span><p>{text}</p></div>}
...
function duration(entry?:string,exit?:string){if(!entry||!exit)return "—";const [eh,em]=entry.split(":").map(Number),[xh,xm]=exit.split(":").map(Number);let minutes=xh*60+xm-(eh*60+em);if(minutes<0)minutes+=1440;return minutes>=60?`${Math.floor(minutes/60)}h ${minutes%60}m`:`${minutes}m`}
